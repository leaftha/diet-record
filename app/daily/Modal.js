...
export default function Modal({ setModal, session }) {
    ...
    return (
        <div className={classes.main} onClick={closeModel}>
            <form onClick={(e) => e.stopPropagation()} className={classes.modal} method="POST" action="api/post/daliy">
                <span className={classes.Xpoint} onClick={closeModel}>
                    X
                </span>
                <h1>운동 내용</h1>
                <textarea className={classes.textbox} name="detail" type="text" placeholder="운동내용" />
                ...
                <input style={{ display: 'none' }} name="date" type="text" defaultValue={date} />
